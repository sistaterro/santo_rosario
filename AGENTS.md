...
Solo `es`, `pt`, `en`, `it`, `fr`, `pl` y `de` estan marcados como `available`. El resto queda `pending` para no mostrar oraciones en un idioma equivocado. La etiqueta del selector debe mantenerse siempre como `Language`, incluso cuando la app este traducida.
